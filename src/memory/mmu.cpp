...
MemoryManagementUnit::MemoryManagementUnit(int num_frames, const std::string& replacement_algorithm) 
    : current_page_references(nullptr), 
    ...
    try {
        ...
        pressure_threshold = config.system.thrashing_fault_threshold;
    } catch (...) {
        pressure_threshold = 0.5;
    }
}

void MemoryManagementUnit::set_pressure_callback(std::function<void(double)> callback) {
...
bool MemoryManagementUnit::handle_page_access(int page_id, uint64_t access_counter) {
    ...
    std::function<void(double)> cb_to_invoke = nullptr;
    double rate_to_pass = 0.0;

    // Check fault pressure threshold every 10 accesses
    if (window_access_count >= 10) {
        double current_rate = static_cast<double>(window_fault_count) / window_access_count;
        if (current_rate >= pressure_threshold && pressure_callback) {
            cb_to_invoke = pressure_callback;
            rate_to_pass = current_rate;
        }
        window_access_count = 0;
        window_fault_count = 0;
    }
    
    lock.unlock(); // Unlock before firing callback to prevent re-entrant deadlocks

    if (cb_to_invoke) {
        cb_to_invoke(rate_to_pass);
    }
